  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-display text-2xl">صور الكاروسيل</h2>
          <p className="text-sm text-muted-foreground mt-1">الصور دي بتظهر في كاروسيل "منتجاتنا" على الصفحة الرئيسية. لو فاضي، بيعرض صور المنتجات تلقائياً.</p>
        </div>
      ...
      {images.length === 0 ? (
        <div className="lux-card p-12 text-center">
          <p className="text-muted-foreground text-sm mb-4">لا توجد صور بعد — ارفع صور لتظهر في الكاروسيل.</p>
          <button onClick={() => fileInputRef.current?.click()} disabled={uploading}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
          {images.map((src, idx) => (
            <div key={idx} className="relative group rounded-2xl overflow-hidden border border-border aspect-square bg-white">
